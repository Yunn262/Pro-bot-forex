...
st.set_page_config(page_title="🚀 Bot Trading Pro — Forex & Cripto", layout="wide")
...
def fetch_forex(pair="EUR/USD", limit=200):
    ...
    date_rng = pd.date_range(end=end_time, periods=limit, freq='15T')
    prices = []
    for dt in date_rng:
        try:
            rate = c.get_rate(pair.split("/")[0], pair.split("/")[1], dt)
        except:
            rate = prices[-1] if prices else 1.1
        prices.append(rate)
    df = pd.DataFrame({'ts': date_rng, 'open': prices, 'high': np.array(prices)*1.001,
                       'low': np.array(prices)*0.999, 'close': prices, 'volume': np.random.randint(100,500,size=limit)})
    df.set_index('ts', inplace=True)
    return df
...
# =================== INTERFACE ===================
st.title("🚀 Bot Trading Pro — Forex & Cripto (Versão PRO)")

market_choice = st.sidebar.selectbox("Escolha o mercado", ["Criptomoeda", "Forex"])
if market_choice == "Criptomoeda":
